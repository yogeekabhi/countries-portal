...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Set pending state to true when API call starts
    setIsApiPending(true);

    getCountriesByRegion(regionWiseEndPoint)
      .then((countries) => {
        // Dispatch action to update country list in the store
        dispatch(updateAllCountryListData(countries));
      })
      .finally(() => {
        // Set pending state to false when API call completes
  ...
  return (
    <div className='countries-page'>
      ...
      <div className='container'>
        ...
        {/* Country List Section with API Pending State */}
        {isApiPending && !allCountries.length ? (
          <div className='countries-loading' role='status' aria-live='polite'>
            <Ellipsis className='loading-icon' size={28} />
          </div>
        ) : (
          <CountryListPaginate pageSize={12} isApiPending={isApiPending} />
        )}
      </div>
